...
export const useAssetController = () => {
  ...
  const handleCreate = () => {
    navigate('/masterdata/assets/create');
  };
  ...
  return {
    ...
    renderListMobileView: () => (
      <ListInfinteScroll
        ...
        pageSize={limit}
        filters={currentFilters}
        onEdit={(item) => navigate(`/masterdata/assets/edit/${item.id}`)}
        onDetail={(item) => navigate(`/masterdata/assets/detail/${item.id}`)}
        onDelete={(item) => navigate(`/masterdata/assets/delete/${item.id}`)}
