...
export const useBranchesController = () => {
  ...
  const { exportToCSV, isExporting } = useExportCSV({
    ...
    filename: `branches_${moment().format('YYYY-MM-DD')}`,
  ...
  const handleCreate = () => {
    navigate('/masterdata/branches/create');
  };
  ...
  return {
    ...
    renderListMobileView: () => (
      <ListInfinteScroll
        ...
        pageSize={limit}
        filters={currentFilters}
        onEdit={(item) => navigate(`/masterdata/branches/edit/${item.id}`)}
        onDetail={(item) => navigate(`/masterdata/branches/detail/${item.id}`)}
        onDelete={(item) => navigate(`/masterdata/branches/delete/${item.id}`)}
