...
export const useCategoriesController = () => {
	...
	const { exportToCSV, isExporting } = useExportCSV({
		...
		filename: `categories_${moment().format("YYYY-MM-DD")}`,
	...
	const handleCreate = () => {
		navigate("/masterdata/categories/create");
	};
	...
	return {
		...
		renderListMobileView: () => (
			<ListInfinteScroll
				...
				pageSize={limit}
				filters={currentFilters}
				onEdit={(item) => navigate(`/masterdata/categories/edit/${item.id}`)}
				onDetail={(item) => navigate(`/masterdata/categories/detail/${item.id}`)}
				onDelete={(item) => navigate(`/masterdata/categories/delete/${item.id}`)}
