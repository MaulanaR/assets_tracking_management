...
export const useDepartmentsController = () => {
  ...
  const handleCreate = () => {
    navigate('/masterdata/departments/create');
  };
  ...
  return {
    ...
    renderListMobileView: () => (
      <ListInfinteScroll
        ...
        pageSize={limit}
        filters={currentFilters}
        onEdit={(item) => navigate(`/masterdata/departments/edit/${item.id}`)}
        onDetail={(item) =>
          navigate(`/masterdata/departments/detail/${item.id}`)
        }
        onDelete={(item) =>
          navigate(`/masterdata/departments/delete/${item.id}`)
        }
