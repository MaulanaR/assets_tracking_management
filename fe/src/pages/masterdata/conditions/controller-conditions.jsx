...
export const useConditionsController = () => {
  ...
  const { exportToCSV, isExporting } = useExportCSV({
    ...
    filename: `conditions_${moment().format('YYYY-MM-DD')}`,
  ...
  const handleCreate = () => {
    navigate('/masterdata/conditions/create');
  };
  ...
  return {
    ...
    renderListMobileView: () => (
      <ListInfinteScroll
        ...
        pageSize={limit}
        filters={currentFilters}
        onEdit={(item) => navigate(`/masterdata/conditions/edit/${item.id}`)}
        onDetail={(item) =>
          navigate(`/masterdata/conditions/detail/${item.id}`)
        }
        onDelete={(item) =>
          navigate(`/masterdata/conditions/delete/${item.id}`)
        }
