...
export const useCategoriesController = () => {
  ...
  const handleCreate = () => {
    navigate('/master-data/categories/create');
  };
  ...
  return {
    ...
    renderListMobileView: () => (
      <ListInfinteScroll
        ...
        pageSize={per_page}
        filters={currentFilters}
        onEdit={(item) => navigate(`/master-data/categories/edit/${item.id}`)}
        onDetail={(item) =>
          navigate(`/master-data/categories/detail/${item.id}`)
        }
        onDelete={(item) =>
          navigate(`/master-data/categories/delete/${item.id}`)
        }
