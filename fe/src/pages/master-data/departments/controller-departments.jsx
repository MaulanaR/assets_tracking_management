...
export const useDepartmentsController = () => {
  ...
  const { exportToCSV, isExporting } = useExportCSV({
    ...
    filename: `departments_${moment().format('YYYY-MM-DD')}`,
  ...
  const handleCreate = () => {
    navigate('/master-data/departments/create');
  };
  ...
  return {
    ...
    renderListMobileView: () => (
      <ListInfinteScroll
        queryKey="mobile-departments"
        endpoint={ENDPOINTS}
        pageSize={limit}
        filters={currentFilters}
        onEdit={(item) => navigate(`/master-data/departments/edit/${item.id}`)}
        onDetail={(item) =>
          navigate(`/master-data/departments/detail/${item.id}`)
        }
        onDelete={(item) =>
          navigate(`/master-data/departments/delete/${item.id}`)
        }
