...
export const useEmployeesController = () => {
  ...
  const handleCreate = () => {
    navigate('/master-data/employees/create');
  };
  ...
  return {
    ...
    renderListMobileView: () => (
      <ListInfinteScroll
        ...
        pageSize={per_page}
        filters={currentFilters}
        onEdit={(item) => navigate(`/master-data/employees/edit/${item.id}`)}
        onDetail={(item) =>
          navigate(`/master-data/employees/detail/${item.id}`)
        }
        onDelete={(item) =>
          navigate(`/master-data/employees/delete/${item.id}`)
        }
