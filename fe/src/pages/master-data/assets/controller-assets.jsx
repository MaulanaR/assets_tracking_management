...
export const useAssetController = () => {
  ...
  const { exportToCSV, isExporting } = useExportCSV({
    ...
    filename: `assets_${moment().format('YYYY-MM-DD')}`,
  ...
  const handleCreate = () => {
    navigate('/master-data/assets/create');
  };
  ...
  return {
    ...
    renderListMobileView: () => (
      <ListInfinteScroll
        ...
        pageSize={limit}
        filters={currentFilters}
        onEdit={(item) => navigate(`/master-data/assets/edit/${item.id}`)}
        onDetail={(item) => navigate(`/master-data/assets/detail/${item.id}`)}
        onDelete={(item) => navigate(`/master-data/assets/delete/${item.id}`)}
