...
import { DEFAULT_PER_PAGE, expandedRowRender, getBreadcrumbItems, getColumns } from './constant';
import { useMaintenanceAssetsController } from './controller-maintenance-assets';
...
const ContentMaintenanceAssets = () => {
  const {
    selectedRow,
    screens,
    initialData,
    ...
    handleExportCSV,
    navigate,
  } = useMaintenanceAssetsController();
        ...
        <Card>
          <Space size={'small'} direction="vertical" style={{ display: 'flex' }}>
            <Flex justify="space-between" wrap="wrap">
              <Space size={'small'}>
                <Title level={2}>Asset Maintenance</Title>
            ...
            <Table
              ...
              columns={columns}
              title={() => TitleTableRender({ selectedLength: selectedRow.length })}
              pagination={{
                responsive: true,
                current: currentPage,
                onChange: onChange,
              ...
              expandable={{ expandedRowRender, rowExpandable: (_) => true }}
            />
          </Space>
        </Card>
      )}
...
export default ContentMaintenanceAssets;